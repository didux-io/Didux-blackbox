...
func (rds *DatabaseInstance) Delete(data interface{}) error {
	...
	ret := rds.db.Del(keyValue)
	err := ret.Err()
	if err == nil && name == peerName {
		ret := rds.db.ZRem(peerIndexName, keyValue)
		err = ret.Err()
	}
	return err
}
...
func (rds *DatabaseInstance) Find(fieldname string, value interface{}, to interface{}) error {
	...
	if key == fieldname {
		ret := rds.db.Get(GetKey(name, value))
		str, err := ret.Result()
		if err != nil {
...
func (rds *DatabaseInstance) Save(data interface{}) error {
	...
	keyValue := GetKey(name, value)
	ret := rds.db.Set(keyValue, bytesValue, -1)
	err = ret.Err()
	if err == nil && name == peerName {
		score := float64(tagged.(*Peer).NextUpdate.Unix())
		ret := rds.db.ZAdd(peerIndexName, redis.Z{Score: score, Member: keyValue})
		err = ret.Err()
	}
	return err
}
...
func (rds *DatabaseInstance) All(instances interface{}) error {
	...
	var cursor uint64
	keys, _, err := rds.db.Scan(cursor, GetKey(name, "*"), 128).Result()
	if err != nil {
		return err
	}
...
func (rds *DatabaseInstance) AllPeers() (*[]types.Peer, error) {
	var cursor uint64
	keys, _, err := rds.db.Scan(cursor, GetKey(peerName, "*"), 128).Result()
	if err != nil {
		return nil, err
	}
...
func (rds *DatabaseInstance) GetNextPeer(postpone time.Duration) (*types.Peer, error) {
	ret := rds.db.ZRange(peerIndexName, 0, 0)
	err := ret.Err()
	var peer types.Peer
	if err == nil {
		list, err := ret.Result()
...
func DBOpen(filename string, log *logrus.Entry) (*DatabaseInstance, error) {
	...
	if err == nil {
		...
		if err == nil {
			...
			log.WithError(err).Fatal("Unable to connect to redis cluster error: ", err)
		} else {
			log.WithError(err).Fatal("Unable to parse redis cluster config file: ", filename, ", error: ", err)
		}
	} else {
		log.WithError(err).Fatal("Could not open redis cluster config file: ", filename, ", error: ", err)
	}
	return nil, err
}
